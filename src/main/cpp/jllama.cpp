        ...
        if (!params.sparams.grammar.empty()) {
...

            {
                ...
                if (it != params.sparams.logit_bias.end() && it->second == -INFINITY) {
                    jllama_log_callback(GGML_LOG_LEVEL_WARN,
    ...
    if (
        ...
        if (llama->params.sparams.n_probs > 0) {
            ...
            if (probs_pos < probs_stop_pos) {
                probs_output = std::vector<completion_token_output>(llama->generated_token_probs.begin() + probs_pos,
...
Java_de_kherud_llama_LlamaModel_getAnswer(JNIEnv *env, jobject obj, jstring prompt, jobject params) {
