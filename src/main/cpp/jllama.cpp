...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *) {
  ...
  if (!(c_llama_model && c_llama_iterator && c_infer_params && c_model_params &&
        c_standard_charsets && c_output && c_string && c_hash_map && c_map &&
        c_set && c_entry && c_iterator && c_integer && c_float && c_log_level &&
  ...
  m_get_bytes =
      env->GetMethodID(c_string, "getBytes", "(Ljava/lang/String;)[B");
  m_entry_set = env->GetMethodID(c_map, "entrySet", "()Ljava/util/Set;");
  m_set_iterator =
      env->GetMethodID(c_set, "iterator", "()Ljava/util/Iterator;");
  ...
  m_map_put = env->GetMethodID(
      c_map, "put", "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;");
  m_int_value = env->GetMethodID(c_integer, "intValue", "()I");
  m_float_value = env->GetMethodID(c_float, "floatValue", "()F");
  m_biconsumer_accept = env->GetMethodID(
      c_biconsumer, "accept", "(Ljava/lang/Object;Ljava/lang/Object;)V");

  if (!(m_get_bytes && m_entry_set && m_set_iterator && m_iterator_has_next &&
        m_iterator_next && m_entry_key && m_entry_value && m_map_put &&
        m_int_value && m_float_value && m_biconsumer_accept)) {
    goto error;
  }
  ...
  f_logit_bias =
      env->GetFieldID(c_infer_params, "logitBias", "Ljava/util/Map;");
  ...
  f_frequency_penalty =
      env->GetFieldID(c_infer_params, "frequencyPenalty", "F");
  ...
  f_antiprompt =
      env->GetFieldID(c_infer_params, "antiPrompt", "[Ljava/lang/String;");
  ...
  f_lora_adapter =
      env->GetFieldID(c_model_params, "loraAdapter", "Ljava/lang/String;");
  f_lora_base =
      env->GetFieldID(c_model_params, "loraBase", "Ljava/lang/String;");
  ...
  if (!(f_model_pointer && f_iter_has_next && f_iter_n_generated &&
        f_iter_token_index)) {
    goto error;
  }
  if (!(f_n_predict && f_n_keep && f_n_probs && f_logit_bias && f_top_k &&
        f_top_p && f_tfs_z && f_typical_p && f_temperature &&
        f_repeat_penalty && f_repeat_last_n && f_frequency_penalty &&
        f_presence_penalty && f_penalize_nl && f_ignore_eos && f_mirostat &&
        f_mirostat_tau && f_mirostat_eta && f_beam_search && f_n_beams &&
        f_grammar && f_antiprompt && f_infer_seed && f_tokenize_special)) {
    goto error;
  }
  if (!(f_n_threads && f_model_seed && f_n_ctx && f_n_batch && f_n_gpu_layers &&
        f_main_gpu && f_tensor_split &&
        //          f_rope_freq_base && f_rope_freq_scale && f_mul_mat_q &&
        //          f_f16_kv && f_logits_all && f_vocab_only &&
        f_rope_freq_base && f_rope_freq_scale && f_f16_kv && f_logits_all &&
        f_vocab_only && f_use_mmap && f_use_mlock && f_embedding &&
        f_lora_adapter && f_lora_base && f_memory_f16 && f_mem_test &&
        //          f_numa && f_verbose_prompt)) {
        f_verbose_prompt)) {
    goto error;
  }

  f_utf_8 = env->GetStaticFieldID(c_standard_charsets, "UTF_8",
                                  "Ljava/nio/charset/Charset;");
  f_log_level_debug =
      env->GetStaticFieldID(c_log_level, "DEBUG", "Lde/kherud/llama/LogLevel;");
  f_log_level_info =
      env->GetStaticFieldID(c_log_level, "INFO", "Lde/kherud/llama/LogLevel;");
  f_log_level_warn =
      env->GetStaticFieldID(c_log_level, "WARN", "Lde/kherud/llama/LogLevel;");
  f_log_level_error =
      env->GetStaticFieldID(c_log_level, "ERROR", "Lde/kherud/llama/LogLevel;");

  if (!(f_utf_8 && f_log_level_debug && f_log_level_info && f_log_level_warn &&
        f_log_level_error)) {
    goto error;
  }
  ...
  o_log_level_debug = (jobject)env->NewGlobalRef(
      env->GetStaticObjectField(c_log_level, f_log_level_debug));
  o_log_level_info = (jobject)env->NewGlobalRef(
      env->GetStaticObjectField(c_log_level, f_log_level_info));
  o_log_level_warn = (jobject)env->NewGlobalRef(
      env->GetStaticObjectField(c_log_level, f_log_level_warn));
  o_log_level_error = (jobject)env->NewGlobalRef(
      env->GetStaticObjectField(c_log_level, f_log_level_error));

  if (!(o_utf_8 && o_log_level_debug && o_log_level_info && o_log_level_warn &&
        o_log_level_error)) {
    goto error;
  }
...
static void jllama_log_callback(enum ggml_log_level level, const char *text,
  ...
  env->CallVoidMethod(g_log_callback, m_biconsumer_accept, java_log_level,
                      java_text);
...
static std::string parse_jstring(JNIEnv *env, jstring java_string) {
  const jbyteArray string_bytes =
      (jbyteArray)env->CallObjectMethod(java_string, m_get_bytes, o_utf_8);
...
// and do the conversion in Java. Unfortunately, there isn't a nice/standardized
// way to do this conversion in C++
...
struct jllama_context {
  ...
  completion_token_output nextToken() {
    ...
    if (embd.size() >= (size_t)n_ctx) {
      ...
      llama_kv_cache_seq_add(ctx, 0, params.n_keep + 1 + n_discard, n_past,
                             -n_discard);
...
static gpt_params parse_model_params(JNIEnv *env, jobject jparams,
                                     jstring java_file_path) {
  gpt_params params;
...
static void setup_infer_params(JNIEnv *env, jllama_context *llama,
                               jobject jparams) {
  ...
  params.antiprompt.clear();
  jobjectArray antiprompt =
      (jobjectArray)env->GetObjectField(jparams, f_antiprompt);
...
static void setup_answering(JNIEnv *env, jllama_context *llama, jstring prompt,
                            jobject params) {
...
static void setup_infilling(JNIEnv *env, jllama_context *llama, jstring prefix,
...
JNIEXPORT void JNICALL Java_de_kherud_llama_LlamaModel_loadModel(
    JNIEnv *env, jobject obj, jstring file_path, jobject jparams) {
...
JNIEXPORT void JNICALL Java_de_kherud_llama_LlamaModel_newInfillIterator(
    JNIEnv *env, jobject obj, jstring prefix, jstring suffix, jobject params) {
  jllama_context *llama = getModelContext(env, obj);
...
JNIEXPORT jobject JNICALL Java_de_kherud_llama_LlamaModel_getNext(
    JNIEnv *env, jobject obj, jobject iter) {
  jllama_context *llama = getModelContext(env, obj);
  ...
  if (stop_pos != std::string::npos) {
    ...
    is_stop_full = false;
    stop_pos =
        llama->findStoppingStrings(str_test, token_text.size(), STOP_PARTIAL);
  }
  ...
  if (stop_pos == std::string::npos ||
    ...
    if (llama->params.sparams.n_probs > 0) {
      ...
      size_t probs_pos =
          std::min(sent_token_probs_index, llama->generated_token_probs.size());
  ...
  return env->NewObject(c_output, cc_output, token_with_probs.tok, jbytes,
                        o_probabilities);
}
...
jbyteArray JNICALL Java_de_kherud_llama_LlamaModel_getInfill(
    JNIEnv *env, jobject obj, jstring prefix, jstring suffix, jobject params) {
  jllama_context *llama = getModelContext(env, obj);
...
JNIEXPORT jfloatArray JNICALL Java_de_kherud_llama_LlamaModel_embed(
    JNIEnv *env, jobject obj, jstring java_prompt) {
  jllama_context *llama = getModelContext(env, obj);
  ...
  env->SetFloatArrayRegion(java_embedding, 0, embedding.size(),
...
JNIEXPORT jintArray JNICALL Java_de_kherud_llama_LlamaModel_encode(
  ...
  env->SetIntArrayRegion(java_tokens, 0, tokens.size(),
...
JNIEXPORT jbyteArray JNICALL Java_de_kherud_llama_LlamaModel_decodeBytes(
    JNIEnv *env, jobject obj, jintArray java_tokens) {
  jllama_context *llama = getModelContext(env, obj);
...
JNIEXPORT void JNICALL Java_de_kherud_llama_LlamaModel_delete(JNIEnv *env,
                                                              jobject obj) {
  jllama_context *llama = getModelContext(env, obj);
  delete llama;
}
...
// */
// static void setup_batch_completion(JNIEnv *env, jllama_context *llama,
// jobjectArray prompts, jobject params) {
//    // llama->prompt = parse_jstring(env, prompt);
...
jobjectArray Java_de_kherud_llama_LlamaModel_batchComplete(
    JNIEnv *env, jobject obj, jobjectArray prompts, jobject inferenceParams) {
  jllama_context *llama = getModelContext(env, obj);
